...
router.post("/users/sign_up", async (req, res, next) => {
  try {
    ...
    if (!regex.test(sign_up_id)) return res.status(400).json({ message: "아이디는 영어 소문자, 숫자로만 이루어져야 합니다." });
    if (isExistUser) return res.status(409).json({ message: "이미 존재하는 아이디입니다." });
    if (password !== password_confirm) return res.status(400).json({ message: "비밀번호가 비밀번호 확인과 일치하지 않습니다." });
    if (password.length < 6) return res.status(400).json({ message: "비밀번호가 6자리 이상 이여야 합니다." });
...
router.post("/users/sign_in", async (req, res, next) => {
  try {
    ...
    if (!(await bcrypt.compare(password, isExistUser.password))) return res.status(401).json({ message: "비밀번호가 같지 않습니다." });
